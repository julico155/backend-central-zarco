...
export async function takeSnapshot(
  client: Queryable,
  options: { realCashSessionId?: string } = {},
): Promise<Snapshot> {
  ...
  for (const row of tables) {
    ...
    const { rows } =
      name === 'cash_register_sessions' && options.realCashSessionId
        ? // Único cambio esperado en toda la base: el correlativo `next_order_number` de la
          // caja real que usa el E2E. Esa fila se huella SIN esa columna; las demás, completas.
          await client.query(
            `select count(*)::int as n,
                    md5(coalesce(string_agg(
                      case when t.id = $1::uuid then (to_jsonb(t) - 'next_order_number')::text else t::text end,
                      '|' order by t.id::text), '')) as h
               from "cash_register_sessions" t`,
            [options.realCashSessionId],
          )
        : await client.query(
